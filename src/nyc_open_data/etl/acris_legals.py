...
def load_acris_legals(engine: Optional[Engine] = None, max_rows: Optional[int] = None) -> None:
    config: DatasetConfig = DATASETS["acris_legals"]
    ...
    run = start_run(engine, "acris_legals")

    try:
        print(f"[acris_legals] Loading dataset '{config.name}' ({config.dataset_id})")
        ...
        if df.empty:
            raise RuntimeError("No rows returned from ACRIS Legals")

        print(f"[acris_legals] DataFrame shape: {df.shape}")
        df = normalize_columns(df)

        write_dataframe(df, config, engine, if_exists="replace", run_id=run.run_id)

        finish_run_success(engine, run, rows=len(df))
        print("[acris_legals] Done.")
